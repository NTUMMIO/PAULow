...
def train_model(model, dataset, kf, batch_size, num_epochs, early_stopping, model_name=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        model_instance = model().to(device)  # fresh model for each fold
        ...
        optimizer = optim.Adam(model_instance.parameters(), lr=0.00001)

        train_losses, val_losses, val_ious, val_dices = [], [], [], []

        with tqdm(total=num_epochs, desc=f"FOLD {fold + 1}", ncols=150) as fold_bar:
            for epoch in range(num_epochs):
                ...
                with torch.no_grad():
                    for images, masks, _ in val_loader:
                        ...
                        if epoch % 5 == 0:
                            save_sample_images(fold, epoch, images.cpu(), masks.cpu(), torch.sigmoid(outputs_resized).cpu(), model_name)
                ...
                if early_stopping.check_early_stop(val_loss):
                    ...
                    fold_bar.n = fold_bar.total
                    fold_bar.last_print_n = fold_bar.total
                    fold_bar.update(0)
                    break

        fold_model_path = os.path.join("utils/temp_files/output", f"{model_name}_fold_{fold + 1}.pth")
        torch.save(model_instance.state_dict(), fold_model_path)

        fold_train_losses.append(train_losses)
        fold_val_losses.append(val_losses)
        fold_val_ious.append(ious)
        fold_val_dices.append(dices)

    print("Cross-Validation Complete!\n")

    return fold_train_losses, fold_val_losses, fold_val_ious, fold_val_dices

def plot_loss_curves(fold_train_losses, fold_val_losses):
    plt.figure(figsize=(14, 6))
    plt.subplot(1, 2, 1)
    for i, losses in enumerate(fold_train_losses):
    ...
    plt.savefig('saved_fig/pbatten_training_curve.png')
    plt.show()
